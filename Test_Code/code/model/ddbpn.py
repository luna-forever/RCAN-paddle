...
class DenseProjection(nn.Layer):
...
class DDBPN(nn.Layer):
    def __init__(self, args):
        ...
        self.upmodules = nn.LayerList()
        self.downmodules = nn.LayerList()
        channels = nr
    ...
    def forward(self, x):
        ...
        for i in range(self.depth - 1):
            if i == 0:
                ...
                l = paddle.concat(l_list, 1)
            h_list.append(self.upmodules[i](l))
            l_list.append(self.downmodules[i](paddle.concat(h_list, 1)))
        
        h_list.append(self.upmodules[-1](paddle.concat(l_list, 1)))
        out = self.reconstruction(paddle.concat(h_list, 1))
